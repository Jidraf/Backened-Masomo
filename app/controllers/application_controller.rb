class ApplicationController < Sinatra::Base
  ...
  get '/students' do
    ...
    all_students.to_json(only: [:id, :student_name, :student_email, :course_id, :teacher_id])
  end

  get '/teachers' do
    all_teachers = Teacher.all
    all_teachers.to_json(only: [:id, :teacher_name, :specialization, :teacher_gender])
  end

  get '/courses' do
    all_courses = Course.all
    all_courses.to_json(only: [:id, :course_name, :course_fees,:course_duration])
  end

  get '/students/:id' do
    single_student = Student.find(params[:id])
    single_student.to_json(only: [:id, :student_name, :student_email], include: {
